...
─────────────
  ...
  1 hour
  2 hours
  Custom…            NSAlert prompt: 30s / 45m / 2h / 1d, up to 7 days
─────────────
  Prevent ▸  ✓ Display (-d)
             ✓ Idle (-i)
               Disk (-m)
...
─────────────
  Launch at Login    omitted entirely when run unbundled
─────────────
  Quit
```
...
└── Contents/
    ├── Info.plist                  LSUIElement=true, bundle id, version, icon
    ├── MacOS/caffeinate-ui         the release binary
    └── Resources/AppIcon.icns      generated by scripts/make-icon.swift
```

The script runs `swift build -c release`, creates those paths, copies the binary
and plist in, generates the icon, and ad-hoc signs the result. It can then be
dragged to `/Applications` and launched from Spotlight.

The icon is drawn from Core Graphics paths rather than reusing the menubar's SF
Symbol, because Apple's SF Symbols license does not permit using symbols in app
icons. It is generated at bundle time rather than committed, so it cannot drift
from the script that draws it.

The ad-hoc signature (`codesign --sign -`) exists for launch-at-login:
`SMAppService` identifies a login item by its code signature and rejects an
entirely unsigned bundle. Ad-hoc is all Command Line Tools can produce, and it
is sufficient — verified by registering and unregistering against a real
...
- **Persisting the login-item choice in-app.** Not needed — `SMAppService`
  already stores it, and `LoginItem.isEnabled` reads the live system status, so
  a local copy could only disagree with it.
